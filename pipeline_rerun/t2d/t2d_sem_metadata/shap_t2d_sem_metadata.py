...
MODEL_FILES = [
    r"C:\Users\Rafael Fonseca\Desktop\Mestrado\Ano2\ProjetoMestrado\parte_2\output\t2d_dataset\gans_augmented\best_model_gans_augmented.pkl",
    r"C:\Users\Rafael Fonseca\Desktop\Mestrado\Ano2\ProjetoMestrado\parte_2\output\t2d_dataset\gans\best_model_gans.pkl",
    r"C:\Users\Rafael Fonseca\Desktop\Mestrado\Ano2\ProjetoMestrado\parte_2\output\t2d_dataset\smote\best_model_smote.pkl",
    r"C:\Users\Rafael Fonseca\Desktop\Mestrado\Ano2\ProjetoMestrado\parte_2\output\t2d_dataset\upsampling\best_model_upsampling.pkl",
]

OUTDIR = Path("t2d_shap_reports"); OUTDIR.mkdir(exist_ok=True)
RANDOM_STATE = 42
...
#keep Country as categorical; only numeric-ize true numeric cols
for c in df.columns:
    if c not in ("healthy", "Country"):
        df[c] = pd.to_numeric(df[c], errors="coerce")

# double check if it exists
if "Country" not in df.columns:
    raise ValueError("Expected 'Country' column in the data.")

#drop rows with missing numeric values
df = df.dropna().sample(frac=1, random_state=RANDOM_STATE).reset_index(drop=True)

#double check if drop happened as it should
if len(df) == 0:
...
for pkl in MODEL_FILES:
    ...
    # --- PREPROCESSING (encoder is required) ---
    encoder = pipe["encoder"]
    scaler  = pipe["scaler"]
    feat_names = pipe["final_feature_names"]

    if encoder is None:
        raise ValueError("This script expects an encoder in the saved pipeline, but got None.")

    # 1) split numeric vs categorical
    if "Country" not in X_test.columns:
        raise ValueError("Column 'Country' is missing but the pipeline expects it.")

    X_num = X_test.drop(columns=["Country"])
    X_cat = encoder.transform(X_test[["Country"]]).toarray()

    # 2) scale numeric part (fit was done during training; here we only transform)
    X_num_scaled = scaler.transform(X_num)

    # 3) rebuild the full feature matrix exactly like during training
    X_all = pd.concat(
        [
            pd.DataFrame(X_num_scaled, columns=X_num.columns),
            pd.DataFrame(X_cat, columns=encoder.get_feature_names_out(["Country"]))
        ],
        axis=1
    )

    # If pre-selection order was saved, enforce it before selecting finals
    if "pre_selection_feature_names" in pipe and pipe["pre_selection_feature_names"]:
        precols = pipe["pre_selection_feature_names"]
        X_all = X_all.loc[:, precols]

    # 4) select the exact training features (names + order)
