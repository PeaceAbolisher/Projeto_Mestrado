...
print("\n[INFO] Starting GANS-Balanced Model Selection Pipeline For Hypertension...\n")
# --- Model Setup ---
models = {
...
balanced_parts = [train_df]  # start with all real samples

# For each class, generate enough synthetic samples to reach max_count
for cls, count in class_counts.items():
    if count < max_count:
        samples_needed = max_count - count
        print(f"Balancing class {cls}: generating {samples_needed} samples")

        # Subset for this class only
        ...
        # 'bp_class' is categorical, all others are numerical
        ...
        # Fit CTGAN on this class and sample the missing rows
        synthesizer = CTGANSynthesizer(metadata, epochs=300)
        synthesizer.fit(cls_df)
        synthetic_cls = synthesizer.sample(samples_needed)

        balanced_parts.append(synthetic_cls)

# Concatenate original + synthetic rows → balanced training set
...
print("Model saved to     : 'hypertension_multi_class_best_model_gans.pkl'")
print("=" * 40)
...
joblib.dump(best_model_pipeline, 'hypertension_multi_class_best_model_gans.pkl')