...
print("\n[INFO] Starting GANS-Balanced And Augmentation Model Selection Pipeline For Hypertension...\n")
# --- Model Setup ---
models = {
    ...
    if count < max_count:
        ...
        for col in cls_df.columns:
            metadata.update_column(
                column_name=col,
                sdtype='categorical' if col == label_col else 'numerical'
            )
...
# Final training sets
X_train = train_df.drop(columns=['bp_class'])
y_train = train_df['bp_class']
        ...
        for base_name, base_model in rfe_base_models.items():
            ...
            for name, model in models.items():
                ...
                if hasattr(grid, "predict_proba"):
                    # For multiclass, use the full probability matrix (N, n_classes)
...
print("Model saved to     : 'hypertension_multi_class_best_model_gans_augmented.pkl'")
print("=" * 40)
...
joblib.dump(best_model_pipeline, 'hypertension_multi_class_best_model_gans_augmented.pkl')